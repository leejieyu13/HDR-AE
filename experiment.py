...
def set_data(hdr0, hdr, EV0, EV, opts):
    ...
    new_img0 = torch.cat((hist0, exp_t0), 1)
    new_img = torch.cat((hist, exp_t), 1)
    images = torch.cat((new_img0, new_img),1)
    return images

def forward_ldr(opts, model, data):
    ...
    image_data = set_data(hdr0, hdr1, EV0, EV1, opts) #batch?
    EV_loss_function = hist_loss.hist_loss()

    for (k, hdr2) in enumerate(hdr_seq): 
    ...
    return torch.mean(loss_hist_seq), loss_hist_seq

        
def forward_hist(opts, model, data):
    """ Evaluate the forward pass of the parameter estimator model. """
    ...
    hdr0 = data['img_seq'][0].to(opts.device) # [B, C, W, H]
    hdr1 = data['img_seq'][1].to(opts.device)
    image_data = set_data(hdr0, hdr1, EV0, EV1, opts)
    EV_loss_function = hist_loss.hist_loss()

    len_seq = len(hdr_seq)
    for (k, hdr2) in enumerate(hdr_seq):
        EV2, _, h = model(image_data, h = h)
        hdr2 = hdr2.to(opts.device)
    ...
    return (loss_sum_EV)/len_seq, EV_seq

def model_test(opts, model, data):
    """ Evaluate the model and test loss without optimizing. """
    loss_hist = torch.FloatTensor([0])
    with torch.no_grad():
        loss_EV, EVs  = forward_hist(opts, model, data)
        if opts.joint_learning:
            loss_hist, _ = forward_ldr(opts, model, data)
    return EVs, loss_EV, loss_hist

def get_errors(loss_p):
    """ Return a dictionary of the current errors. """
    error_dict = {'Loss': loss_p.item()}
...
def train(opts, model, train_data, val_data, num_epochs, resume_from_epoch=None):    
    ...
    for epoch in range(initial_epoch, num_epochs + 1):
        ...
        for i,data in enumerate(train_loader):
            ...
            if opts.joint_learning:
                if epoch %2 ==0:
                    loss, EVs = forward_hist(opts, model, data)
                else:
                    loss, EVs  = forward_ldr(opts, model, data)
        ...
        for data in val_loader:
            EVs, loss_EV, loss_hist = model_test(opts, model, data)
            target = torch.transpose(data['target'][:,2:], 0, 1).to(opts.device)
        ...
        print('Val errors: ', val_errors) #EV loss
        print('Val mse: ', val_mse)
        print('Val hist loss: ', val_hist)
        # print('Val mse: ', mseLoss(torch.cat(EV_seq), torch.cat(gt_seq).to(opts.device)))
